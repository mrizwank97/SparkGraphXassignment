    ...
    public static void shortestPathsExt(JavaSparkContext ctx) {
        ...
        ops.pregel(new Tuple2<Integer,List<Object>>(Integer.MAX_VALUE, Lists.newArrayList()), // initialMsg
                ...
                .foreach(v -> {
                    ...
                    System.out.println("Minimum cost to get from "+labels.get(1l)+" to "+labels.get(vertex._1)+" is "+vertex._2._2.stream().map(key -> labels.get(key)).filter(Objects::nonNull).collect(Collectors.toList())+" with cost "+vertex._2._1);
                });
    }
}